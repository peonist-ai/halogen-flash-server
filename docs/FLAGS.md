...
| `HALOGEN_KV_POOL_POSITIONS` | `3 x HALOGEN_CTX`, capped at 1,048,576 | Attention positions resident across all conversations; `HALOGEN_CTX` is the most one request may use. A request reserves its prompt plus `max_tokens` positions when admitted and waits in arrival order when they are not free. The default 786,432 holds three full-length conversations, about 42 GB on the device, measured with all three resident and generating; 524,288 is about 35 GB and 1,048,576 needs `HALOGEN_MAX_TOK=16384` (about 41 GB). Generation speed follows each conversation's own length, not the pool. The pool takes RAM the page cache would otherwise keep for the model's n-gram table, so a prompt whose rows are not cached reads them from disk first; a smaller pool leaves more cache. |
